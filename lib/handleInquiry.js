...
async function handleInquiry(body) {
  ...
  if (!emailOk && !smsOk) {
    const emailErr = emailResult.status === 'rejected' ? emailResult.reason?.message : '邮件未发送';
    const smsErr = smsResult.status === 'rejected' ? smsResult.reason?.message : '短信未发送';
    return {
      status: 502,
      body: {
        ok: false,
        message: `通知发送失败：${[emailErr, smsErr].filter(Boolean).join('；')}`,
      },
    };
  }
